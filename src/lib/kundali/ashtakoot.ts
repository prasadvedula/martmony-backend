...
export interface AshtakootResult {
  ...
  maxScore: number
  percentage: number
  recommendation: 'EXCELLENT' | 'GOOD' | 'AVERAGE' | 'BELOW_AVERAGE' | 'POOR'
  recommendationText: string
  mangalDosha: MangalDoshaResult
}

export interface MangalDoshaResult {
  groomHasMangal: boolean | null
  brideHasMangal: boolean | null
  doshaPresent: boolean
  doshaText: string
}

// ─── 1. Varna (max 1) ────────────────────────────────────────────────────────
const VARNA_RANK: Record<VarnaType, number> = {
  BRAHMIN: 4,
  KSHATRIYA: 3,
  VAISHYA: 2,
  SHUDRA: 1,
...
function calcVarna(groom: Nakshatra, bride: Nakshatra): KootScore {
...
// ─── 2. Vasya (max 2) ────────────────────────────────────────────────────────
function calcVasya(groom: Nakshatra, bride: Nakshatra): KootScore {
  const scored = getVasyaScore(groom.vasya, bride.vasya)
  return {
    name: 'Vasya',
    nameHindi: 'वश्य',
    maxPoints: 2,
...
function calcTara(groom: Nakshatra, bride: Nakshatra): KootScore {
...
// ─── 4. Yoni (max 4) ─────────────────────────────────────────────────────────
const YONI_POINTS: Record<string, number> = {
  SAME: 4,
  FRIENDLY: 3,
  NEUTRAL: 2,
  ENEMY: 0,
...
function calcYoni(groom: Nakshatra, bride: Nakshatra): KootScore {
  const relation = getYoniRelation(groom.yoni as YoniAnimal, bride.yoni as YoniAnimal)
...
const GRAHA_POINTS: Record<string, number> = {
  FRIEND_FRIEND: 5,
  FRIEND_NEUTRAL: 4,
  NEUTRAL_FRIEND: 4,
  NEUTRAL_NEUTRAL: 3,
...
function calcGrahaMaitri(groom: Nakshatra, bride: Nakshatra): KootScore {
  const r1 = getPlanetRelation(groom.lord, bride.lord)
  const r2 = getPlanetRelation(bride.lord, groom.lord)
  const key = `${r1}_${r2}`
...
// ─── 6. Gana (max 6) ─────────────────────────────────────────────────────────
const GANA_POINTS: Record<GanaType, Record<GanaType, number>> = {
  DEVA:      { DEVA: 6, MANUSHYA: 5, RAKSHASA: 1 },
  MANUSHYA:  { DEVA: 5, MANUSHYA: 6, RAKSHASA: 0 },
  RAKSHASA:  { DEVA: 1, MANUSHYA: 0, RAKSHASA: 6 },
}

function calcGana(groom: Nakshatra, bride: Nakshatra): KootScore {
...
function rashiCount(from: string, to: string): number {
...
function calcBhakoot(groom: Nakshatra, bride: Nakshatra): KootScore {
...
// ─── 8. Nadi (max 8) ─────────────────────────────────────────────────────────
function calcNadi(groom: Nakshatra, bride: Nakshatra): KootScore {
...
// ─── Mangal Dosha ─────────────────────────────────────────────────────────────
export function checkMangalDosha(groomHasMangal: boolean | null, brideHasMangal: boolean | null): MangalDoshaResult {
...
export function calculateAshtakoot(
  groomNakshatraName: string,
  brideNakshatraName: string,
  groomMangal: boolean | null = null,
  brideMangal: boolean | null = null
): AshtakootResult {
  const groom = getNakshatraByName(groomNakshatraName)
  const bride = getNakshatraByName(brideNakshatraName)

  if (!groom || !bride) {
    throw new Error(
      `Nakshatra not found: ${!groom ? groomNakshatraName : brideNakshatraName}`
    )
  }
  ...
  let recommendation: AshtakootResult['recommendation']
