...
import { ParsedProfileDraft } from './pdf-parser'

function parseDDMMYYYY(val: string): string | undefined {
  const m = val.match(/(\d{2})\/(\d{2})\/(\d{4})/)
  if (!m) return undefined
  return `${m[3]}-${m[2]}-${m[1]}`  // ISO YYYY-MM-DD
}

function parseHeight(val: string): number | undefined {
...
function parseSalary(val: string): number | undefined {
  if (!val || /student|studying|nil|no|home/i.test(val)) return undefined
  const lacPa = val.match(/([\d.]+)\s*Lac/i)
  if (lacPa) return parseFloat(lacPa[1])
...
function inferState(city: string): string | undefined {
  ...
  if (c.match(/hyd|secunderabad|warangal|karimnagar|khammam|nalgonda|nizamabad|kukatpally|manikonda|ameerpet|dilsukh|alwal|nagole|boduppal|jeedimetla|miyapur|gachibowli|kondapur|madhapur/)) return 'Telangana'
  if (c.match(/vijayawada|guntur|vizag|visakhapatnam|nellore|tirupati|tirupathi|eluru|nuziveedu|tenali|badvel|kadapa|ongole|kurnool|bhimavaram|rajahmundry|kakinada|anakapalli|srikakulam|chittoor|hindupur|anantapur|chintalapudi|pulivendula|cuddapah/)) return 'Andhra Pradesh'
  if (c.match(/bangalore|bengaluru|mysore|mangalore/)) return 'Karnataka'
  if (c.match(/chennai|coimbatore|madurai/)) return 'Tamil Nadu'
  if (c.match(/mumbai|pune|nagpur/)) return 'Maharashtra'
  if (c.match(/delhi|noida|gurgaon/)) return 'Delhi'
  if (c.match(/usa|uk|london|australia|canada|singapore/)) return 'Abroad'
  return undefined
}

function extractNakshatra(raw: string): string {
...
export function isDailyEditionFormat(text: string): boolean {
  return /ENROL ID\s*:(?:DAILY|SUNDAY)/i.test(text) && /Nakshatram and Padam/i.test(text)
}

export function parseDailyEditionText(text: string): ParsedProfileDraft[] {
  // Split on block headers
  const blocks = text.split(/(?=DATE:\s*\d{2}-\d{2}-\d{4}\s*---\s*ENROL ID)/i)
    .filter(b => b.trim().length > 80)

  return blocks.map(b => parseBlock(b)).filter((d): d is ParsedProfileDraft => d !== null)
}

function parseBlock(raw: string): ParsedProfileDraft | null {
  const warnings: string[] = []

  // Gender
  let gender: 'MALE' | 'FEMALE' | undefined
  if (/\bBride\b/i.test(raw) && !/\bBridegroom\b/i.test(raw)) gender = 'FEMALE'
  else if (/\b(?:Bridegroom|Groom)\b/i.test(raw)) gender = 'MALE'
  else if (/వరుని/.test(raw)) gender = 'MALE'
  else if (/వధువు|వధ్ువు/.test(raw)) gender = 'FEMALE'
  ...
  let mangalDosha: boolean | undefined
  const kujaMatch = raw.match(/Kuja Dosham\s*\n+\s*([^\n]{1,60})/)
  if (kujaMatch) {
    const kv = kujaMatch[1].trim()
    if (/లేదు|no/i.test(kv)) mangalDosha = false
    else if (/ఉంది|ఉుంది|yes/i.test(kv)) mangalDosha = true
  }
  ...
  if (/ఆంధ్ర|ఆుంధ్ర/.test(raw)) currentState = 'Andhra Pradesh'
  else if (/తెలంగాణ|తేలంగాణ|తెలంగాన/.test(raw)) currentState = 'Telangana'
  else if (/కర్నాటక/.test(raw)) currentState = 'Karnataka'
  else if (/తమిళనాడు/.test(raw)) currentState = 'Tamil Nadu'
  else if (currentCity) currentState = inferState(currentCity)
  ...
  if (!fullName) warnings.push('Name not found')
  if (!gender) warnings.push('Gender not found')
  if (!dateOfBirth) warnings.push('Date of birth not found')
