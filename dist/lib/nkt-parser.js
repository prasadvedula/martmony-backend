...
exports.parseNKTEntry = parseNKTEntry;
exports.parseNKTText = parseNKTText;
exports.parseNKTBuffer = parseNKTBuffer;
exports.isNKTFormat = isNKTFormat;
// ── Height code ─────────────────────────────────────────────────────────────
// 508 = 5'8" = 172cm, 600 = 6'0" = 183cm
function parseNKTHeight(code) {
...
function toAnnualLpa(amount, unit) {
    const u = unit.toLowerCase();
    if (u.includes('inr') && u.includes('lakh'))
        return amount;
    if (u.includes('us dollar') || u.includes('usd'))
        return +(amount * 0.0012).toFixed(2); // rough
    if (u.includes('uk pound') || u.includes('gbp'))
        return +(amount * 0.0015).toFixed(2);
    if (u.includes('uae dirham') || u.includes('aed'))
        return +(amount * 0.00033).toFixed(2);
...
function normaliseNakshatra(raw) {
...
function inferState(text) {
    ...
    if (/andhra|ap\b/.test(t))
        return 'Andhra Pradesh';
    if (/telangana/.test(t))
        return 'Telangana';
    if (/karnataka|bangalore|bengaluru/.test(t))
        return 'Karnataka';
    if (/tamil|chennai/.test(t))
        return 'Tamil Nadu';
    if (/maharashtra|mumbai|pune/.test(t))
        return 'Maharashtra';
    if (/delhi|noida|gurgaon/.test(t))
        return 'Delhi';
    if (/usa|united states/.test(t))
        return 'USA';
    if (/uk|united kingdom/.test(t))
        return 'UK';
    if (/uae|dubai/.test(t))
        return 'UAE';
    if (/gujarat|ahmedabad/.test(t))
        return 'Gujarat';
    if (/rajasthan/.test(t))
        return 'Rajasthan';
    return undefined;
...
function parseRequirements(req) {
    ...
    for (const p of parts) {
        ...
        if (st && !prefStates.includes(st))
            prefStates.push(st);
    }
    return { prefAgeMin, prefAgeMax, prefStates };
}
// ── Main entry parser ─────────────────────────────────────────────────────────
function parseNKTEntry(regCode, bodyText, sectionGender) {
    const warnings = [];
    ...
    if (!dobMatch)
        warnings.push('Could not parse date of birth');
    // Birth place
    ...
    const parseConfidence = presentCount === 5 ? 'HIGH' : presentCount >= 3 ? 'MEDIUM' : 'LOW';
    return {
...
async function parseNKTBuffer(buffer) {
