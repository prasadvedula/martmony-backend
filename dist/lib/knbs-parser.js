...
exports.parseKNBSBlock = parseKNBSBlock;
exports.parseKNBSText = parseKNBSText;
exports.parseKNBSBuffer = parseKNBSBuffer;
function parseHeightKNBS(val) {
...
function inferState(city) {
    ...
    if (c.includes('hyd') || c.includes('secunderabad') || c.includes('warangal') ||
        ...
        c.includes('alwal') || c.includes('nagole') || c.includes('boduppal'))
        return 'Telangana';
    if (c.includes('vijayawada') || c.includes('guntur') || c.includes('vizag') ||
        ...
        c.includes('yellamanchili') || c.includes('ongole') || c.includes('kurnool'))
        return 'Andhra Pradesh';
    if (c.includes('chennai') || c.includes('bangalore') || c.includes('bengaluru'))
        return c.includes('chennai') ? 'Tamil Nadu' : 'Karnataka';
    if (c.includes('mumbai') || c.includes('pune') || c.includes('nagpur'))
        return 'Maharashtra';
    if (c.includes('delhi') || c.includes('noida') || c.includes('gurgaon'))
        return 'Delhi';
    if (c.includes('usa') || c.includes('texas') || c.includes('new york') || c.includes('austin'))
        return 'USA';
    if (c.includes('raipur'))
        return 'Chhattisgarh';
    if (c.includes('roorkela') || c.includes('bhubaneswar'))
        return 'Odisha';
    return undefined;
}
function parseKNBSBlock(raw, gender) {
...
function parseKNBSText(text) {
    ...
    for (const line of lines) {
        ...
        if (upper.includes('BOYS')) {
            currentGender = 'MALE';
            continue;
        }
        if (upper.includes('GIRLS')) {
...
async function parseKNBSBuffer(buffer) {
