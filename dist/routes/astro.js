...
function nakToRasi(constellation) {
...
function nakToName(constellation) {
    return constellation.split('-')[0].trim();
}
function rasiToEng(rasi) {
...
async function vedaPost(method, stdTime, lat, lon, locName) {
    ...
    if (!res.ok)
        throw new Error(`VedAstro HTTP ${res.status}`);
    const json = await res.json();
    if (json?.Status !== 'Pass')
        throw new Error(`VedAstro: ${json?.Payload ?? 'error'}`);
    return json.Payload;
}
...
function calcHouse(planetSign, refSign) {
    if (!planetSign || !refSign)
        return 0;
    return ((planetSign - refSign + 12) % 12) + 1;
}
function analyzeMangalDosha(marsSignNum, lagnaSignNum, moonSignNum, jupiterSignNum) {
    ...
    if (lagnaSignNum === 11)
        cancellationReasons.push('Aquarius Lagna — natural cancellation');
    if (lagnaSignNum === 4)
        cancellationReasons.push('Cancer Lagna — dosha not applicable');
...
router.post('/calculate', async (req, res) => {
    ...
    try {
        ...
        for (const row of (allConst?.AllPlanetConstellation ?? [])) {
            constellations[row.Planet.toLowerCase()] = row.AllPlanetConstellation ?? '';
        }
        // Nakshatra
        ...
        // Rasi
        const moonSignEng = moonSignPayload?.MoonSignName ?? '';
        const rasi = moonSignEng ? (ENG_TO_RASI[moonSignEng] ?? moonSignEng) : nakToRasi(moonConst);
        // Lagna
        const lagnaEng = lagnaSignPayload?.LagnaSignName ?? '';
        ...
        // Build planets map
        const buildRasi = (planet) => nakToRasi(constellations[planet] ?? '');
        return res.json({
            success: true,
            data: {
                nakshatra,
